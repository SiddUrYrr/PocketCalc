...
st.markdown(
    ...
    .kbd{background:#f1f5f9;border-radius:8px;padding:10px}
    .result-box{background:#111827;color:#fff;padding:12px;border-radius:10px}
    </style>
    """,
...
with st.container():
    ...
    # Header
    st.markdown(
        '<div class="phone-header"><div style="display:flex;align-items:center"><span class="dot red"></span><span class="dot yellow"></span><span class="dot green"></span></div><div style="font-weight:700">PocketCalc</div><div class="muted">v1.1</div></div>',
        unsafe_allow_html=True,
    )
        ...
        if st.button("Compute Matrix Operation"):
            try:
                ...
                if op == "Add":
                    ...
                    st.success(f"Rank(A) = {np.linalg.matrix_rank(A)}")
                    R = None
                if R is not None:
                    st.code(pretty_matrix(R))
    ...
    with tab2:
        ...
        if st.button("Compute Determinant & Inverse"):
            try:
                ...
                st.markdown('<div class="kbd">Determinant</div>', unsafe_allow_html=True)
                st.write(round(det, 10))
                if abs(det) < 1e-12:
                    st.warning("Matrix is singular. No inverse.")
                else:
                    inv = np.linalg.inv(M)
                    st.markdown('<div class="kbd">Inverse</div>', unsafe_allow_html=True)
                    st.code(pretty_matrix(inv))
            except Exception as e:
                st.error(f"Error: {e}")
